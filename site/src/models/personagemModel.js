...
function registrarTempoDeJogo( fkUsuario) {
    console.log("TEMPO DE JOGO REGISTRADO COM SUCESSO! \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function registrarTempoDeJogo()", mes, tempoDeJogo, fkUsuario);
    var instrucao = `
        INSERT INTO qtdJogos (mes, tempoDeJogo, fkUsuario) VALUES ('${mes}', '${tempoDeJogo}', '${fkUsuario}');
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
